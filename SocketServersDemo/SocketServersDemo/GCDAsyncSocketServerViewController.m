//
//  GCDAsyncSocketServerViewController.m
//  SocketServersDemo
//
...
#import "GCDAsyncSocketServerViewController.h"
#import <GCDAsyncSocket.h>

//图片
#define kImageDataType 0x00000001
#define kVideoDataType 0x00000002

@interface GCDAsyncSocketServerViewController ()<GCDAsyncSocketDelegate>
...
@implementation GCDAsyncSocketServerViewController
...
- (IBAction)didClickSendAction:(id)sender {
    ...
    // 数据类型 data
    // 2.拼接指令类型(4~7:指令)
    unsigned int command = kImageDataType;
    ...
    for (GCDAsyncSocket *soc in self.clientSockets) {
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag{
    ...
    if (self.dataM.length == self.totalSize) {
        //丢包的现象
        NSLog(@"数据已经接收完成");
        if (self.currentCommandId == kImageDataType) {//图片
            [self saveImage];
        }else if  (self.currentCommandId == kVideoDataType){
            [self saveVideo];
        }
        // 响应客户端
    }
